...
module ProjectivePlane.Harmonic.Uniqueness.Case1-A-4-nuevo {a b c d e}
    ...
    module A
      ...
      module B 
       ...
       (Q'∉PR : Q' ∉ join P♯R ) where
        ...
        open import ProjectivePlane.Polygon
        ...
        perspectiveAxe : ArePerspectiveAxe tPQR t'QPS
        perspectiveAxe 
          = toPerspectiveAxe 
            (record
               { axe = join A♯B
        ...
        pc-tPQR-t'QPS = desargues← _ _ perspectiveAxe
    ...
    module A
      ...
      module B 
       ...
       (Q∈P'R' : Q ∈ join H'.P♯R) where
         ...
         pa-tPRS-t'QSR  : ArePerspectiveAxe tPRS t'QSR
         pa-tPRS-t'QSR = desargues _ _ pc-tPRS-t'QSR


         module apa  =  ArePerspectiveAxe pa-tPRS-t'QSR
         ...
         axe≈AB : apa.axe  ≈ join A♯B
         axe≈AB = unq-join _ (⟨ A≈apa02 ⟩⇐ apa.perspective 0♯2) (⟨ B≈apa01 ⟩⇐ apa.perspective 0♯1)


         D≈D' : D ≈ D'
         D≈D' = begin
                     D             
                       ≈⟨ sym $ unq-meet _ meetₗ ( apa.perspective 1♯2 ⇒⟨ axe≈AB ⟩) ⟩
                     meet (AreDistinct.sd♯ apa.areDistinct 1♯2)  
                       ≈⟨ unq-meet _ (meetᵣ ⇒⟨ join-comm ⟩) ( apa.perspective 1♯2  ⇒⟨ axe≈AB ⟩) ⟩
                     D' 
                ∎
...
absurdity₁  A B C A♯B C∈AB HC HC' 
   ...
   where
        ...
        pa-PRS-QSR' : ArePerspectiveAxe tPRS t'QSR
        pa-PRS-QSR' = desargues _ _ pc-tPRS-t'QSR


        module apa  =  ArePerspectiveAxe pa-PRS-QSR'
